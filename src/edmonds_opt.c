...
long long edmonds_opt(Graph *g, int root, int *parent) {
    ...
    while (1) {
        ...
        for (int i = 0; i < m; i++) {
            heap_push(heaps[edges[i].to], edges[i].from, edges[i].weight);
        }
        
        for (int i = 0; i < n; i++) {
            if (i == root) { in[i] = 0; pre[i] = root; continue; }
            while (!heap_empty(heaps[i])) {
                HeapNode top = heap_pop(heaps[i]);
                if (top.vertex == i) continue;
                in[i] = top.key;
                pre[i] = top.vertex;
                break;
            }
...
cleanup:
    free(edges); free(pre); free(in); free(id); free(vis);
    return total;
}